...
const Utils = {
    ...
    // Format date with relative formatting for past and future dates
    formatDate(dateStr) {
        if (!dateStr) return '';

        const date = new Date(dateStr);
        const now = new Date();
        const diffTime = now - date; // Positive if date is in the past, negative if in the future
        ...
        if (diffDays > 0) {
            ...
            // Within last month
            if (absDiffDays < 30) {
                return `${Math.floor(absDiffDays / 7)} weeks ago`;
            }
        ...
        // Future dates
            ...
            // Within next year
            if (absDiffDays < 365) {
                return `In ${Math.floor(absDiffDays / 30)} months`;
            }
